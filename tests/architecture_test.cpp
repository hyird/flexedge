...
int main() {
    ...
    REQUIRE(!std::filesystem::exists(std::filesystem::path(FLEXEDGE_SOURCE_DIR) /
                                     "service/domains/dns_zone/dns_zone.service.h"));
    const auto dnsZoneCommand = source("service/domains/dns_zone/dns_zone_command.service.h");
    REQUIRE(dnsZoneCommand.contains("class DnsZoneCommandService final"));
    REQUIRE(!dnsZoneCommand.contains("dns_zone_read.service.h"));
    REQUIRE(!dnsZoneCommand.contains("dnsZoneReadService()"));
    REQUIRE(!dnsZoneCommand.contains("selectColumns"));
    REQUIRE(!dnsZoneCommand.contains("fillDnsZone"));
    REQUIRE(dnsZoneCommand.contains("validateSystemManagedRecords"));
    REQUIRE(dnsZoneCommand.contains("SYSTEM_MANAGED_RECORD"));
    ...
    const auto dnsZoneController = source("service/domains/dns_zone/dns_zone.controller.h");
    REQUIRE(dnsZoneController.contains("dns_zone_command.service.h"));
    REQUIRE(dnsZoneController.contains("dns_zone_read.service.h"));
    REQUIRE(!dnsZoneController.contains("dns_zone.service.h"));
    REQUIRE(dnsZoneController.contains("dnsZoneCommandService().create"));
    REQUIRE(dnsZoneController.contains("dnsZoneReadService().list"));
    REQUIRE(!std::filesystem::exists(std::filesystem::path(FLEXEDGE_SOURCE_DIR) /
                                     "service/domains/cluster/cluster.service.h"));
    const auto clusterCommand = source("service/domains/cluster/cluster_command.service.h");
    REQUIRE(clusterCommand.contains("class ClusterCommandService final"));
    REQUIRE(!clusterCommand.contains("cluster_read.service.h"));
    REQUIRE(!clusterCommand.contains("clusterReadService()"));
    REQUIRE(!clusterCommand.contains("SELECT cluster.id, cluster.name"));
    REQUIRE(clusterCommand.contains("publishClusterRelease"));
    REQUIRE(clusterCommand.contains("reconcileZoneTransition"));
    ...
    const auto clusterController = source("service/domains/cluster/cluster.controller.h");
    REQUIRE(clusterController.contains("cluster_command.service.h"));
    REQUIRE(clusterController.contains("cluster_read.service.h"));
    ...
    REQUIRE(!nodeReleaseArtifact.contains("atomic_compare_exchange_strong_explicit"));
    const auto runtimeState = source("node/runtime/runtime_state.h");
    REQUIRE(runtimeState.contains("std::atomic<std::shared_ptr<const CompiledConfig>> config_"));
    REQUIRE(!runtimeState.contains("std::atomic_load("));
    REQUIRE(!runtimeState.contains("std::atomic_store("));
    const auto originHealthSource = source("node/data/origin_health.h");
    REQUIRE(originHealthSource.contains("std::atomic<std::shared_ptr<const StateMap>> states_"));
    REQUIRE(!originHealthSource.contains("atomic_load_explicit"));
    REQUIRE(!originHealthSource.contains("atomic_compare_exchange_weak_explicit"));
    const auto tlsContext = source("node/data/tls_context.h");
    REQUIRE(tlsContext.contains("std::atomic<std::shared_ptr<const TlsContextSet>> contexts_"));
    REQUIRE(!tlsContext.contains("std::atomic_load("));
    REQUIRE(!tlsContext.contains("std::atomic_store("));
