...
export function Search({
  ...
  return (
    <Button
      ...
      variant='outline'
      className={cn(
        'group relative h-8 w-8 flex-none justify-center rounded-md bg-muted/25 px-0 text-sm font-normal text-muted-foreground shadow-none hover:bg-accent sm:w-40 sm:flex-1 sm:justify-start sm:px-3 sm:pe-12 md:flex-none lg:w-52 xl:w-64',
        className
      )}
    ...
    >
      <SearchIcon
        ...
        size={16}
      />
      <span className='sr-only sm:not-sr-only sm:ms-4'>{placeholder}</span>
