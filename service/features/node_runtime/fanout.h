...
namespace service::node_runtime::fanout {
...
class Hub final {
  public:
    class Subscription final {
      public:
        ...
        receiveFor(std::chrono::steady_clock::duration timeout, ruvia::StopToken stopToken) const {
            if (!receiver_) {
                throw std::logic_error("node runtime subscription receiver is not initialized");
            }
    ...
    [[nodiscard]] Subscription subscribe(const ruvia::WorkerHandle& worker,
        ...
        if (tenantId.empty()) {
            throw std::invalid_argument("node runtime subscription requires a tenant");
        }
